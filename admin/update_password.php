...
if(isset($_POST['submit'])){
    $current_password = $admin->filter($_POST['current_password']);
    $new_password = $admin->filter($_POST['new_password']);
    $confirm_password = $admin->filter($_POST['confirm_password']);
    if(password_verify($current_password,$adminInfo->password)){
        if($new_password == $confirm_password){
            $pass = password_hash($new_password,PASSWORD_DEFAULT);
            $updated_at = date('Y-m-d H:i:s');
            $admin->updatePassword($pass,$updated_at,$adminInfo->id);
            $admin->addLog('Password updated',$adminInfo->id,$updated_at);
